package me.kanuunankuula.debugStick.permissions;

public class PropertyPermissions {

    public static final String BASE = "debugstick.property";
    public static final String USE = "debugstick.use";

    public static final String WATERLOGGED = BASE + ".waterlogged";
    public static final String FACING = BASE + ".facing";
    public static final String AXIS = BASE + ".axis";
    public static final String OPEN = BASE + ".open";
    public static final String POWERED = BASE + ".powered";
    public static final String HALF = BASE + ".half";
    public static final String ROTATION = BASE + ".rotation";
    public static final String LIT = BASE + ".lit";
    public static final String AGE = BASE + ".age";
    public static final String LEVEL = BASE + ".level";

    public static final String ATTACHED = BASE + ".attached";
    public static final String BOTTOM = BASE + ".bottom";
    public static final String CONDITIONAL = BASE + ".conditional";
    public static final String DISARMED = BASE + ".disarmed";
    public static final String DRAG = BASE + ".drag";
    public static final String ENABLED = BASE + ".enabled";
    public static final String EXTENDED = BASE + ".extended";
    public static final String EYE = BASE + ".eye";
    public static final String FALLING = BASE + ".falling";
    public static final String HANGING = BASE + ".hanging";
    public static final String HAS_BOTTLE = BASE + ".has_bottle";
    public static final String HAS_RECORD = BASE + ".has_record";
    public static final String INVERTED = BASE + ".inverted";
    public static final String IN_WALL = BASE + ".in_wall";
    public static final String OCCUPIED = BASE + ".occupied";
    public static final String PERSISTENT = BASE + ".persistent";
    public static final String SHORT = BASE + ".short";
    public static final String SIGNAL_FIRE = BASE + ".signal_fire";
    public static final String SNOWY = BASE + ".snowy";
    public static final String TRIGGERED = BASE + ".triggered";
    public static final String UNSTABLE = BASE + ".unstable";

    public static final String HINGE = BASE + ".hinge";
    public static final String SHAPE = BASE + ".shape";
    public static final String TYPE = BASE + ".type";
    public static final String DISTANCE = BASE + ".distance";
    public static final String PART = BASE + ".part";
    public static final String ATTACHMENT = BASE + ".attachment";
    public static final String BITES = BASE + ".bites";
    public static final String CAN_SUMMON = BASE + ".can_summon";
    public static final String CHARGES = BASE + ".charges";
    public static final String MODE = BASE + ".mode";
    public static final String NOTE = BASE + ".note";
    public static final String PICKLES = BASE + ".pickles";
    public static final String POWER = BASE + ".power";
    public static final String SLAB_TYPE = BASE + ".slab_type";
    public static final String STAGE = BASE + ".stage";
    public static final String VARIANT = BASE + ".variant";
    public static final String EGGS = BASE + ".eggs";
    public static final String HATCH = BASE + ".hatch";
    public static final String DELAY = BASE + ".delay";
    public static final String INSTRUMENT = BASE + ".instrument";

    public static final String UP = BASE + ".up";
    public static final String DOWN = BASE + ".down";
    public static final String NORTH = BASE + ".north";
    public static final String SOUTH = BASE + ".south";
    public static final String EAST = BASE + ".east";
    public static final String WEST = BASE + ".west";

    public static final String SHRIEKING = BASE + ".shrieking";
    public static final String MOISTURE = BASE + ".moisture";
    public static final String FACE = BASE + ".face";

    private PropertyPermissions() {
    }
}